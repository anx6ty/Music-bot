...
def build_now_playing_embed(guild_id, paused=False):
    ...
    if not song:
        embed = discord.Embed(title="🔮 Nothing Playing", color=PURPLE)
        return embed

    queue_count = len(queues.get(guild_id, []))

    embed = discord.Embed(
        title="⏸️ Spell Paused" if paused else "🔮 Now Casting",
    ...
    embed.add_field(name="⏱️ Duration", value=song.get('duration_str', 'Unknown'), inline=True)
    embed.add_field(name="🔁 Loop", value=get_loop_label(guild_id), inline=True)
    embed.add_field(
        name="📃 Up Next",
        value=f"{queue_count} song(s) queued" if queue_count else "Queue is empty",
        inline=True
    )
...
class MusicView(discord.ui.View):
    def __init__(self, guild_id):
        ...
        if mode == "track":
            loop_label, loop_style = "🔂 Loop: Track", discord.ButtonStyle.success
        elif mode == "queue":
            loop_label, loop_style = "🔁 Loop: Queue", discord.ButtonStyle.success
        else:
            loop_label, loop_style = "🔁 Loop: Off", discord.ButtonStyle.secondary

        loop_btn = discord.ui.Button(label=loop_label, style=loop_style)
        loop_btn.callback = self.loop_callback
        self.add_item(loop_btn)

        stop_btn = discord.ui.Button(label="⏹️ Stop", style=discord.ButtonStyle.danger)
