...
def build_progress_bar(elapsed, duration, length=30):
    """Build a modern progress bar using ▰ and ▱."""
    if not duration:
        return "▰" * length  # full bar if no duration
...
# ============================================================
# NOW PLAYING EMBED – Clean, with modern progress bar
# ============================================================
...
# ============================================================
# SEEK FUNCTIONALITY – Fixed (no stop)
# ============================================================
...
async def seek_audio(guild_id, delta):
    ...
    # Update start_time to reflect new position
    ...
    # Create new FFmpeg source with -ss
    ffmpeg_opts = {
    ...
    # Define after callback that continues the song logic
    def after_seek(error):
        if error:
            print(f"[SEEK ERROR] {error}")
        # After seeked part finishes, we need to continue to next song
        # Since we only seeked within the same song, we should resume playback of the same song
        # Actually we just need to continue playing the rest of the song.
        # We'll mimic the original after callback behavior.
        guild = bot.get_guild(guild_id)
        if guild:
            # We need to call play_next to proceed to next song when this segment finishes.
            # But this segment is the rest of the song, so after it finishes, the song is done.
            # We can just call play_next directly.
            asyncio.run_coroutine_threadsafe(play_next(guild, guild.get_channel(song.get('_channel_id', 0))), bot.loop)
