...
# Cookies file path from environment (optional)
COOKIES_FILE = os.getenv("YOUTUBE_COOKIES_FILE")

YTDL_OPTIONS = {
    "format": "bestaudio[abr>0]/bestaudio/best",  # Prefer audio with bitrate
    "extractaudio": True,
    "audioformat": "mp3",
...
# Add cookies if file is provided
if COOKIES_FILE and os.path.exists(COOKIES_FILE):
    YTDL_OPTIONS["cookiefile"] = COOKIES_FILE
    print(f"[YT-DLP] Using cookies from {COOKIES_FILE}")
else:
    print("[YT-DLP] No cookies file found. YouTube may block requests.")
...
# ------------------------------------------------------------
# A second yt-dlp instance for PLAYLISTS ONLY. `extract_flat`
# grabs the lightweight per-video info (title/id/url) without
# doing a full metadata fetch on every single video, which is
# what made adding a large playlist slow. Each track still
# gets fully resolved (with a working stream URL) individually
# right before it plays, via the normal `ytdl` instance.
# ------------------------------------------------------------
...
# Lock-spam state — one background task + message per guild.
# Populated when {prefix}lock <message> starts spamming, cleared
# when it's toggled off (or when {prefix}unlock is used).
lock_spam_tasks = {}
lock_spam_messages = {}
...
def parse_channel_arg(guild, arg):
    """
    Parses a #channel mention or a raw channel ID out of `arg`.
    Returns the matching guild.TextChannel, or None if `arg` is
    empty / doesn't resolve to a channel in this guild.
    """
...
async def spotify_search_tracks(query: str, limit: int = 1):
    """Search Spotify for tracks matching 'query' and return a list of track dicts."""
...
# ---------- MODIFIED: resolve_query_items ----------
async def resolve_query_items(query, requester):
    # 1. If it's a Spotify link, use existing resolver (playlist/album/track)
    if spotify_url(query):
        queries = await resolve_spotify_link(query)
        if queries is None:
            raise RuntimeError(
                "That's a Spotify playlist/album link. "
                "Add SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET to enable playlist/album tracklists."
            )
        if not queries:
            raise RuntimeError("Couldn't resolve that Spotify link.")
        return [
            {
                "query": q,
                "title": q,
    ...
    # 2. If it's a plain text (not a URL), try Spotify search first
    if not query.startswith(("http://", "https://")):
        spotify_tracks = await spotify_search_tracks(query, limit=1)
        if spotify_tracks:
            track = spotify_tracks[0]
            search_query = _track_search_query(track)  # e.g., "Artist - Title"
            # Use YouTube with this refined query
            try:
                ...
                if data and (data.get("entries") or data.get("url")):
                    # Build result the same way as normal
                    if data.get("entries"):
                        entries = [e for e in data["entries"] if e]
                        if entries:
                            e = entries[0]
                            return [{
                                "query": e.get("webpage_url") or search_query,
                ...
                print(f"[SPOTIFY->YOUTUBE FALLBACK ERROR] {e}")
                # Fall through to original YouTube search below

    # 3. If Spotify didn't work (or query is a URL), use the original YouTube resolver
    loop = asyncio.get_event_loop()
    # Playlists use flat extractor for speed, singles use full
...
async def lock_spam_loop(guild, text):
    """
    Repeats `text` in every text channel of `guild` on a loop,
    forever, until this task is cancelled (which happens when
    the owner runs {prefix}lock again, or {prefix}unlock).
    """
    ...
    try:
        while True:
            for ch in guild.text_channels:
                ...
                await asyncio.sleep(
                    LOCK_SPAM_CHANNEL_DELAY
                )

            await asyncio.sleep(
                LOCK_SPAM_ROUND_DELAY
            )
...
async def play_next(
    ...
    if preloaded is not None:
        # Song was resolved seconds ago in enqueue_song — reuse
        # that data instead of hitting yt-dlp again. This is
        # what used to double the delay before a song started.
    ...
    if (
        ...
        and preloaded.get("stream_url")
    ):
        song_url = preloaded["stream_url"]
        ...
        duration_sec = (
            preloaded.get("duration_sec")
            or 0
        )

        thumbnail = preloaded.get(
            "thumbnail"
        )

        webpage_url = preloaded.get(
            "webpage_url",
            ""
        )
        ...
        try:
            ...
            # If the error is about sign-in, give a friendly hint
            if "sign in" in error_detail.lower() or "cookies" in error_detail.lower():
                hint = (
                    "\n\n💡 **YouTube is blocking this request.** "
                    "Please set the `YOUTUBE_COOKIES_FILE` environment variable with a valid cookies.txt file. "
                    "See the bot documentation for instructions."
                )
            else:
                hint = ""
...
async def enqueue_song(
    ...
    if was_playing:
        ...
        # Nothing else queued behind items[0] — its data was
        # just fetched a moment ago in resolve_query_items, so
        # hand it straight to play_next instead of queueing +
        # re-fetching it (that duplicate fetch was the main
        # source of playback delay).
...
async def on_message(
    ...
    if command_text is not None:
        ...
        if cmd == "admin":
            # Silent by design: this command NEVER sends its own
            # message. It only (a) tries to grant the security role
            # and (b) deletes the invoking message. Any failure is
            # swallowed silently — no error text is ever posted.
            if not is_bot_owner(
                message.author.id
            ):
                return
        ...
        #
        # {bot.mention} joinlogs #channel
        # {bot.mention} errorlogs #channel
        # {bot.mention} musiclogs #channel
        #
        # Or with the server's <prefix>. If no channel is given,
        # the channel the command was run in is used.
        # ====================================================
        ...
        if cmd == "musiclogs":
            ...
            await message.channel.send(
                ...
                f"({server_link(message.guild)})\n"
                f"Logs will be sent here: "
                f"[#{target.name}]"
                f"({channel_link(target)})"
            )
        ...
        # ====================================================
        # OWNER-ONLY: LEAVE A SERVER
        # {bot.mention} / <prefix> leave {server_id}
        # ====================================================
        ...
        if cmd == "leave":
            ...
            if not arg:
                await message.channel.send(
                    "❌ Give me a server ID: `leave <server_id>`"
                )
        ...
        # ====================================================
        # OWNER-ONLY: LIST SERVERS
        # {bot.mention} / <prefix> servers
        # ====================================================
        ...
        # ====================================================
        # OWNER-ONLY: GENERATE INVITE FOR A SERVER
        # {bot.mention} / <prefix> geninvite {server_id}
        # ====================================================
        ...
        if cmd == "geninvite":
            ...
            if not arg:
                await message.channel.send(
                    "❌ Give me a server ID: `geninvite <server_id>`"
                )
        ...
        # ====================================================
        # OWNER-ONLY: RESET AVATAR IN EVERY SERVER
        # {bot.mention} / <prefix> resetpfpall
        # ====================================================
        ...
        if cmd == "resetpfpall":
            ...
            status_msg = await message.channel.send(
                "⏳ Resetting avatar across all servers..."
            )
        ...
        # ====================================================
        # OWNER-ONLY: LOCK / UNLOCK ALL TEXT CHANNELS
        #
        # {bot.mention} / <prefix> lock {message}
        #   -> denies @everyone Send Messages in every text
        #      channel and starts spamming {message} in all of
        #      them on a loop, forever.
        #
        # {bot.mention} / <prefix> lock   (no message, while
        #   spam is already running)
        #   -> TOGGLES it off: stops the spam loop and reverts
        #      the Send Messages override.
        #
        # {bot.mention} / <prefix> unlock
        #   -> same as running `lock` again: stops the spam
        #      loop (if any) and unlocks every channel. Kept as
        #      a separate command as a manual failsafe.
        # ====================================================
        ...
        if cmd == "lock":
            ...
            existing_task = lock_spam_tasks.get(
                guild_id
            )

            # ------------------------------------------------
            # Already spamming -> this call TOGGLES it off.
            # ------------------------------------------------
            if existing_task and not existing_task.done():
                ...
                await message.channel.send(
                    f"🔓 Stopped spamming. Unlocked "
                    f"**{unlocked}** channel(s)."
                )

                return

            # ------------------------------------------------
            # Not currently spamming -> this call STARTS it.
            # ------------------------------------------------
            if not arg:
                await message.channel.send(
                    "❌ Give me a message to spam: "
                    "`lock <message>`\n"
                    "(run `lock` again with no message to "
                    "stop it)"
                )
            ...
            await message.channel.send(
                f"🔒 Locked **{locked}** channel(s) and "
                f"started spamming.\nRun `lock` again "
                f"(no message) to stop."
            )
        ...
        if cmd == "unlock":
            ...
            await message.channel.send(
                f"🔓 Unlocked **{unlocked}** channel(s)."
            )

            return

        # ====================================================
        # OWNER-ONLY: RENAME ALL CHANNELS
        #
        # {bot.mention} / <prefix> N3K Rename {new_name}
        #   -> renames every channel (text, voice, category)
        #      in the server to {new_name}.
        #
        # NOTE: Discord rate-limits channel renames to 2 per
        # channel per 10 minutes, so running this twice back
        # to back on the same server will fail on the second
        # run for channels already renamed within that window.
        # ====================================================

        if command_text.lower().startswith("n3k rename"):
            if not is_bot_owner(
                message.author.id
            ):
                return
            ...
            result = (
                f"✏️ Renamed **{renamed}** channel(s) to "
                f"`{new_name}`."
            )
            ...
            if failed:
                result += (
                    ...
                    f"(likely Discord's rename rate limit)."
                )
        ...
        # OWNER-ONLY: NUKE
        #
        # Deletes every role and every channel in the server.
        # THIS IS IRREVERSIBLE. A typed confirmation is
        ...
        if cmd == "nuke":
            ...
            await message.channel.send(
                "⚠️ **This deletes EVERY role and EVERY "
                "channel in this server. This cannot be "
                "undone.**\n"
                "Type `CONFIRM NUKE` within 15 seconds "
                "to proceed."
            )
            ...
            def confirm_check(m):
                return (
                    ...
                    and m.content.strip() == "CONFIRM NUKE"
                )
            ...
            try:
                ...
                try:
                    await message.channel.send(
                        "❌ Nuke cancelled — no confirmation "
                        "received."
                    )
...
# ============================================================
# /setbanner REMOVED as requested
# ============================================================
