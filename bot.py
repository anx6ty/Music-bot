...
def get_spotify_track(url):
    if not spotify:
        raise RuntimeError("Spotify Client ID/Secret are missing.")
    ...
    while results:
        for item in results["items"]:
            ...
            if not track:
                continue
            artists = ", ".join(artist["name"] for artist in track["artists"])
            songs.append(f"{artists} - {track['name']}")
        if results.get("next"):
    ...
    def ask():
        response = ai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a Discord music search assistant. Convert the user's request into a good music search query. Return ONLY the search query."
                },
    ...
    def ask():
        response = ai.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a music recommendation AI. Give exactly 10 music recommendations in format: 1. Song - Artist. Do not write explanations."
                },
                {"role": "user", "content": query}
            ],
            temperature=0.8,
...
async def resolve_music(query):
    ...
    if is_spotify(query) and "/track/" in query:
        name = await loop.run_in_executor(None, get_spotify_track, query)
        result = await search_youtube(name)
...
def create_player_embed(guild_id, paused=False):
    ...
    position = song_positions.get(guild_id, 0)
    ...
    embed.add_field(
        name="Loop",
        value="Off" if not loop_modes.get(guild_id) else loop_modes[guild_id],
        inline=True
    )
    embed.add_field(
        name="Progress",
        value=f"`{format_time(position)}` {bar} `{format_time(duration)}`",
        inline=False
    )
    embed.set_footer(text="10s seek • Playback controls")
...
class MusicView(discord.ui.View):
    ...
    async def back10(self, interaction: discord.Interaction, button: discord.ui.Button):
        await seek_music(interaction, -10)
    ...
    async def pause_play(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not voice:
            await interaction.response.send_message("❌ I'm not in VC.", ephemeral=True)
            return
        ...
        await interaction.response.edit_message(
            embed=create_player_embed(self.guild_id, voice.is_paused()),
            view=self
        )

    @discord.ui.button(label="10s", emoji="⏩", style=discord.ButtonStyle.secondary)
    async def forward10(self, interaction: discord.Interaction, button: discord.ui.Button):
        await seek_music(interaction, 10)

    @discord.ui.button(label="Loop", emoji="🔁", style=discord.ButtonStyle.success)
    async def loop(self, interaction: discord.Interaction, button: discord.ui.Button):
        guild_id = self.guild_id
        current = loop_modes.get(guild_id)
