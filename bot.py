...
async def spotify_search_tracks(query: str, limit: int = 1):
    """Search Spotify for a track. Returns None if credentials missing or error."""
    if not SPOTIFY_CLIENT_ID or not SPOTIFY_CLIENT_SECRET:
        return None
    token = await get_spotify_token()
    if not token:
...
async def resolve_query_items(query, requester):
    """Resolve a query: Spotify link -> track(s), plain text -> Spotify search -> YouTube fallback."""
    # 1. If it's a Spotify link
    if spotify_url(query):
        tracks = await resolve_spotify_link(query)
        if tracks is None:
            # Credentials missing or error – fallback to YouTube
        ...
        # Build items from Spotify tracks
        items = []
        for t in tracks:
            if isinstance(t, dict):
                search_q = _track_search_query(t)
                # Try to get YouTube audio for this track
                try:
                    yt_item = await resolve_youtube(search_q, requester, single=True)
                    if yt_item:
                        # Merge Spotify data into item
                        yt_item[0]["spotify_track"] = t
                        items.extend(yt_item)
                    else:
                        # If YouTube fails, just add a placeholder (shouldn't happen)
                        items.append({
                            "query": search_q,
                    ...
                    # Still add Spotify info without stream_url (will fail playback but log)
                    items.append({
                        "query": search_q,
                        "title": t.get("name", "Unknown"),
                        "thumbnail": t.get("album", {}).get("images", [{}])[0].get("url"),
    ...
    # 2. Plain text – search Spotify first
    if not query.startswith(("http://", "https://")):
        spotify_tracks = await spotify_search_tracks(query, limit=1)
        if spotify_tracks:
            t = spotify_tracks[0]
            search_q = _track_search_query(t)
            # Try YouTube with this refined query
            try:
                ...
                # If YouTube fails, we'll still return a "track" without stream_url (will error later)
                return [{
                    "query": search_q,
                    "title": t.get("name", "Unknown"),
                    "thumbnail": t.get("album", {}).get("images", [{}])[0].get("url"),
    ...
    # 3. Fallback: YouTube direct (or URL)
    return await resolve_youtube(query, requester)

async def resolve_youtube(query, requester, single=False):
    """Resolve a query directly with YouTube (yt-dlp)."""
...
# ============================================================
# PLAYBACK / QUEUE HELPERS (same as before)
# ============================================================
...
def build_now_playing_embed(guild_id, paused=False):
    ...
    if requester:
        embed.set_footer(text=f"🪄 cast by {requester.display_name} · updates every 2s",
                         icon_url=requester.display_avatar.url)
    return embed


# ============================================================
# MUSIC VIEW (Now Playing buttons)
# ============================================================
...
# ============================================================
# QUEUE NOTICE VIEW (with proper callbacks)
# ============================================================
...
# ============================================================
# PLAYBACK FUNCTIONS (play_next, enqueue, etc.)
# ============================================================
...
async def on_interaction(interaction: discord.Interaction):
    ...
    # QueueNoticeView buttons are handled via interaction_check in the view
...
async def on_message(message: discord.Message):
    ...
    if command_text is not None:
        ...
        # Owner-only commands (admin, logs, leave server, nuke, lock, unlock, etc.)
        if cmd == "admin":
            if not is_bot_owner(message.author.id):
                return
            # silent admin role grant (keep existing code)
            # ...
            return
        ...
        # Owner lock/unlock/nuke/rename commands (keep from previous code)
        # For brevity, not rewriting, but they exist in the original.
